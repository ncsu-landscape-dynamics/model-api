...
modelapi <- function(case_study_id, session_id, run_collection_id, run_id) {
  ...
  if (!is.null(run$steering_year)) {
    if (run$steering_year > start_time) {
      ...
      previous_run_occurrence <- previous_output$single_spread_map
      previous_occurrence <- geojsonio::as.json(previous_run_occurrence)
      previous_occurrence <- geojson::as.geojson(previous_occurrence)
      previous_occurrence <- geojsonio::geojson_sp(previous_occurrence)
      previous_occurrence <- spTransform(previous_occurrence, CRS = crs(host))
      occurrence <- raster::rasterize(previous_occurrence, host, fun = "last")
      occurrence[is.na(occurrence)] <- 0
      infected <- raster::as.matrix(occurrence)

      previous_susceptibles <- previous_output$susceptible_map
      previous_susceptible <- geojsonio::as.json(previous_susceptibles)
      previous_susceptible <- geojson::as.geojson(previous_susceptible)
      previous_susceptible <- geojsonio::geojson_sp(previous_susceptible)
      previous_susceptible <- spTransform(previous_susceptible, CRS = crs(host))
      susceptibles <- raster::rasterize(previous_susceptible, host, fun = "last")
      susceptibles[is.na(susceptibles)] <- 0
      susceptible_difference <- raster::as.matrix(susceptibles)
      susceptible <- susceptible_start - susceptible_difference
  ...
  stuff <- foreach::foreach(q = 1:length(years), .packages =c("raster", "geojsonio", "httr"), .export = ls(globalenv())) %dopar% {
    ...
    susceptible_map <- susceptible_run[[q]]
    susceptible_map <- as.integer(susceptible_map)
    susceptible_map[susceptible_map <= 0] <- NA
    names(susceptible_map) <- "outputs"
    susceptible_map <- projectRaster(susceptible_map, crs = CRS("+proj=longlat +datum=WGS84"), method = "ngb")
    susceptible_map <- raster::rasterToPolygons(susceptible_map, n = 4, digits = 4, dissolve = T, na.rm = TRUE)
    storage.mode(susceptible_map$outputs) <- "integer"
    susceptible_map <- geojsonio::geojson_list(susceptible_map, convert_wgs84 = TRUE, geometry = "polygon")
    class(susceptible_map) <- "list"
    ...
    outs$susceptible_map <- susceptible_map
